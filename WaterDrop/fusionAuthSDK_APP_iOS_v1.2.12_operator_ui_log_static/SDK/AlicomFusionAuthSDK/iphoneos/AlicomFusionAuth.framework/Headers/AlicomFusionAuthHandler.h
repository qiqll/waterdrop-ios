...
/**
 *  设置是否使用SDK内置图形认证模块
 *  @note SDK默认内置图形认证功能，如果您不需要使用此功能，可通过此接口进行关闭
 *  @note 客户端本地强制关闭图形认证模块后，图形认证相关功能将与控制台配置断连，无法在控制台重新配置打开。
 *  @param isUseSupply YES:使用图形认证模块  NO:不使用图形认证模块； 默认YES
 */

+ (void)useSDKSupplyCaptchaModule:(BOOL)isUseSupply;
